...
new_d.to_csv(path_or_buf="/media/ubuntu/ANL/Data1_lane_xynew.csv", index=False)


    # plt.scatter(np.round(d['x_utm'].tolist(), 2), np.round(d['y_utm'].tolist(), 2), s=2, color="black", label='original')
    # if len(waste) > 0:
